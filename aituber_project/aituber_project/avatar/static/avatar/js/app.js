...
document.addEventListener("DOMContentLoaded", () => {
    ...
    async function fetchJSON(url, options = {}) {
        ...
        if (!response.ok) {
            const error = new Error(
                data.error || `APIエラーが発生しました。HTTP ${response.status}`
            );
    ...
    async function generateSuperChat() {
        try {
            ...
            const data = await fetchJSON("/api/generate-comment/");

            addCommentToUI(data.comment, "superchat");

            const replyData = await fetchJSON("/api/reply-to-comment/");
    ...
    async function generateSelfIntroduction() {
        try {
            ...
            const data = await fetchJSON("/api/self-introduction/");
    ...
    async function generateNewsTalk() {
        try {
            ...
            const data = await fetchJSON("/api/news-talk/");
    ...
    async function generateWeatherTalk() {
        try {
            ...
            const data = await fetchJSON("/api/weather-talk/");
    ...
    async function sendUserComment() {
        ...
        try {
            ...
            const data = await fetchJSON("/api/user-comment/", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json"
                },
